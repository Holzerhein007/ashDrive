...
def nav():
    ...
    while going == True:
        ...
        elif navigate=="upload":
            global upfile
            upfile = input("Enter full path of file to upload: ")
            upload_file()
        
        elif navigate=="updir":
            global up_dir
            up_dir = input("Enter full path of directory with files to batch upload: ")
            upload_dir()
...
def upload_file():
    ...
    # upload
    media = MediaFileUpload(upfile.strip('\n'), resumable=True)
    file = service.files().create(body=file_metadata, media_body=media, fields='id').execute()
    print("File created, id:", file.get("id"), "Name:", file.get("name"))
    current_folder = str(result[0][3])
    current_folder = current_folder.strip("]'[")
    search_result = search(service, query=f"parents='{current_folder}' and trashed = false")
    current_folder = str(result[0][3])
...
def upload_dir():
    ...
    # authenticate account
    service = get_gdrive_service()
    #open the teext file 'zzz' which contains the files you want to batch upload
    with open(up_dir+'zzz', 'rt') as f:
        for line in f:
            print(line)
            if line == "zzz\n":
                break
        ...
        # upload
            media = MediaFileUpload(up_dir+line.strip('\n'), resumable=True)
            file = service.files().create(body=file_metadata, media_body=media, fields='id').execute()
            print("File created, id:", file.get("id"), "Name:", file.get("name"))
